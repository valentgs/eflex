...
class NetworkResourceSchema(ma.SQLAlchemySchema):
    ...
    network_resource_type_id = fields.Integer(required=True)
...
class NetworkResourceTypeSchema(ma.SQLAlchemySchema):
    """
    NetworkResourceType schema, with validations.
    """
    ...
    class Meta:
        model = NetworkResourceType


class NetworkResourceIdField(MarshmallowClickMixin, fields.Int):
    """Field that deserializes to a NetworkResource and serializes back to an integer."""

    @with_appcontext_if_needed()
    def _deserialize(self, value, attr, obj, **kwargs) -> NetworkResource:
        """Turn a network resource id into a NetworkResource."""
        network_resource = db.session.get(NetworkResource, value)
